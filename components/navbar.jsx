...
export default function Navbar() {
  ...
  const [isSupplier, setIsSupplier] = useState(false); // Supplier holati uchun
    ...
    const checkAuth = () => {
      ...
      if (token && userStr) {
        try {
          ...
          // Groups ichida Supplier borligini tekshirish
          const supplierCheck = user.groups?.some(group => group.name === "Supplier");
          setIsSupplier(supplierCheck);
        } catch (e) {
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center gap-[24px]">
            ...
            <div className="relative">
              ...
              >
                Каталог
                <ChevronDownIcon className={`w-4 h-4 transition-transform duration-200 ${catalogOpen ? "rotate-180" : ""}`} />
              </button>
                        ...
                        categories.map((category) => (
                          ...
                          >
                            ...
                            {category.description && (
                              <div className="text-xs text-gray-500 truncate mt-1">{category.description}</div>
                            )}
                          </Link>
                        ))
                      ) : (
                        <div className="py-2 px-3 text-center text-gray-500">Нет категорий</div>
                      )}
                    </div>
                  </motion.div>
                )}
          ...
          <div className="flex items-center gap-4">
            ...
            {/* Sahifada Supplier bo'lsa Favorites va Cart ko'rinmaydi */}
            {!isSupplier && (
              <>
                {/* Favorites button */}
                <Link href={isLoggedIn ? "/profile/favorites" : "/auth/login"}>
                ...
                {/* Cart button */}
                <Link href={isLoggedIn ? "/profile/zakas" : "/auth/login"}>
                  <div className="w-[47px] h-[47px] bg-gradient-to-br from-[#D8C19A] to-[#C3974C] rounded-[12px] flex items-center justify-center cursor-pointer">
            ...
            {isLoggedIn ? (
              <Link href={isSupplier ? "/supplier/dashboard" : "/profile/dashboard"}>
